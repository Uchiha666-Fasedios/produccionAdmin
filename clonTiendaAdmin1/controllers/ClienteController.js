...
const email_activar = async function(req, res){//async define una función asíncrona,
    ...
    var email=req.params['email'];
        try {//si va todo bien y no hay errores porqe si se pone en la url otro id desconocido captura el error
            var reg  = await Cliente.find({email:email});//findById busco por id
              if (reg) {
                //console.log(reg);
                enviar_email_de_activacion(email);
...
const pass_cambio = async function(req, res){//async define una función asíncrona,
    ...
    const activar_cliente = async function(req, res){//async define una función asíncrona,
        
      
           
             var activar=req.params['activar'];
        try {//si va todo bien y no hay errores porqe si se pone en la url otro id desconocido captura el error
            var cliente  = await Cliente.findById({_id:activar});//findById busco por id
              if (cliente) {
                var reg  = await Cliente.findByIdAndUpdate({_id:activar},{//findByIdAndUpdate busco por id y actualiza
                    activar:'activado'
                    });
...
const enviar_email_de_registro = async function(reg){
    try {
...
const enviar_email_de_activacion = async function(email){
    try {
...
const login_cliente = async function(req,res){
    ...
    }else{//hay correo registrado
       
        let activado= undefined;
        activado  = await Cliente.findOne({email:data.email,activar:'activado'});//busca una direccion q tenga el id del usuario y q principal este en true  

if (activado == undefined) {//no esta activado
res.status(200).send({message: 'El correro no esta activado',email:data.email, activar:'desactivado',data: undefined});
}else{//esta activado
